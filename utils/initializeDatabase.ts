...
// Helper to suppress "database is locked" noise
function logUnlessLocked(error: unknown, context: string) {
...
const getDatabase = async (): Promise<SQLite.SQLiteDatabase> => {
  if (!dbInstance) {
    ...
    // Split PRAGMA calls to avoid "near PRAGMA: syntax error"
...
export const debouncedSafeInitializeDatabase = debounce(async () => {
  try {
    await safeInitializeDatabase();
  } catch (error) {
    console.error("Error in debounced initialize:", error);
  }
}, 3000);
...
const fetchVersionFromSupabase = async (): Promise<string> => {
  ...
  if (!data?.version) throw new Error("No version row returned");
  return data.version;
};

const checkVersionAndSync = async () => {
  const versionFromStorage = await Storage.getItem("version");
  let versionFromSupabase: string;

  try {
    versionFromSupabase = await fetchVersionFromSupabase();
  } catch (error) {
    console.error("Error fetching remote version:", error);
    if (!versionFromStorage) {
      // First-run but cannot fetch remote version: force initial sync
      await dropTables();
      await createTables();
  ...
  if (versionFromSupabase !== versionFromStorage) {
    await dropTables();
    await createTables();
    await fetchAndSyncAllData();
  ...
  try {
    ...
    await db.execAsync(`
      ...
      CREATE TABLE IF NOT EXISTS favorites (
        prayer_id INTEGER NOT NULL,
        user_category_id INTEGER NOT NULL DEFAULT 1,
...
// ... [ other fetchAndSyncX implementations unchanged, using logUnlessLocked in catch blocks ]

// --- Subscriptions & Cleanup ---
const subscriptions: any = [];
export const setupSubscriptions = () => {
  const verSub = supabase
    .channel("version")
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "version" },
      async () => {
        console.log("Version change received!");
        await safeInitializeDatabase();
  ...
  const ppSub = supabase
    .channel("paypal")
    .on(
      "postgres_changes",
      { event: "*", schema: "public", table: "paypal" },
      async () => {
        console.log("PayPal change received!");
        await fetchPayPalLink();
        router.replace("/(tabs)/home");
        Toast.show({
          type: "success",
  ...
  subscriptions.push(verSub, ppSub);
};
...
export const addPrayerToFavorite = async (
  prayerId: number,
  categoryId: number,
): Promise<void> => {
  try {
    const db = await getDatabase();
    await db.runAsync(
      `INSERT OR IGNORE INTO favorites (prayer_id, user_category_id) VALUES (?, ?);`,
      [prayerId, categoryId]
    );
    Toast.show({
      type: "success",
...
export const removePrayerFromFavorite = async (
  ...
  try {
    const db = await getDatabase();
    await db.runAsync(
      `DELETE FROM favorites WHERE prayer_id = ? AND user_category_id = ?;`,
      [prayerId, categoryId]
    );
    Toast.show({
      type: "success",
...
const fetchPayPalLink = async () => {
  try {
    ...
    if (data?.paypal_link) {
      Storage.setItem("paypal", data.paypal_link);
      const db = await getDatabase();
      await db.runAsync(
...
/**
 * Delete a user‐created favorites category (and all its favorites).
 */
export const deleteFavoriteCategory = async (
  ...
  try {
    ...
    Toast.show({
      type: "success",
      text1: i18n.t("categories"),
      text2: i18n.t("toast.categoryDeleted"),
    });
  } catch (error) {
    logUnlessLocked(error, "Error deleting favorite category:");
    Toast.show({
      type: "error",
      text1: i18n.t("toast.error"),
      text2: i18n.t("toast.categoryDeleteError"),
    });
    throw error;
  }
};
