...
// import { defaultWeeklyTodos } from "@/utils/defaultWeeklyTodos";
...
import { ThemedText } from "./ThemedText";
import { Ionicons } from "@expo/vector-icons";
// Remove AsyncStorage import if only used for todos
import { useTranslation } from "react-i18next";
import { useLanguage } from "@/context/LanguageContext";
import EvilIcons from "@expo/vector-icons/EvilIcons";
import {
  ...
  PrayerWithCategory,
  DailyPrayer,
} from "@/utils/types"; // Keep types if used elsewhere
import { ThemedView } from "./ThemedView";
import {
  getDailyPrayerForToday,
  isMonthlyTrue,
...
import { categories } from "@/utils/categories";
// Remove defaultWeeklyTodos and storage key import
// const WEEKLY_TODOS_STORAGE_KEY = "prayer_app_weekly_todos";

// Import the custom hook
import { useWeeklyTodos } from "@/hooks/useWeeklyTodos"; // Adjust path as needed
...
const HomeScreen = () => {
  ...
  // --- Use the custom hook for todo state and logic ---
  const {
    weeklyTodos,
    isLoading: isLoadingTodos, // Rename to avoid conflict if other loading states exist
    toggleTodo,
    addTodo: addTodoFromHook, // Rename to avoid conflict with local handler
    deleteTodo: deleteTodoFromHook, // Rename to avoid conflict with local handler
    undoAllTodosForDay,
  ...
  // --- Remove Todo Loading/Saving useEffects (handled by the hook) ---
  // useEffect(() => { loadTodos(); ... }, [language]); // REMOVED
  // useEffect(() => { saveTodos(); ... }, [weeklyTodos]); // REMOVED
  // --------------------------------------------------------------------

  // --- Local handlers for UI actions (modals, input) ---

  // Handle adding a todo (called by modal button)
  const handleAddTodo = (): void => {
    if (newTodo.trim() && selectedDay !== null) {
      addTodoFromHook(selectedDay, newTodo.trim()); // Call the hook's function
  ...
  const handleConfirmDelete = (): void => {
    ...
    if (dayIndex !== null && todoId !== null) {
      deleteTodoFromHook(dayIndex, todoId); // Call the hook's function
    }
  ...
  // --- Helper functions for UI text (remain the same) ---
  const getDayNames = (): string[] => {
    return (
      (t("days.short", { returnObjects: true }) as string[]) || [
        "Mon",
